...
SELECT * FROM emp WHERE empno = 1019;
INSERT INTO emp (empno, deptno, job, ename) VALUES(1019, 50, '사원', '송중기');

SELECT * FROM emp WHERE empno = 1019;
INSERT INTO emp (empno, deptno, job, ename) VALUES(1019, 50, '사원', '송중기')
ON DUPLICATE key UPDATE ename = '송송송', job = '대리', deptno = 30;
...
SELECT @empno1 := MAX(empno) + 1 FROM emp;

INSERT INTO emp (empno, ename) VALUES (@empno1, '강민경')
                                    , (@empno1 := @empno1 +1, '아이린')
												, (@empno1 := @empno1 +1, '김갑환');
...
-- 작업 1
SAVEPOINT tran1;
DELETE FROM emp1 WHERE ename = '이병헌';
SELECT * FROM emp1;
...
-- 작업 2
SAVEPOINT tran2 ;
UPDATE emp1 SET job = '과장' WHERE job = '대리';
SELECT * FROM emp1 WHERE job IN('과장', '대리') OR (ename = '이병헌');
SELECT * FROM emp1;
...
-- 작업 3
SAVEPOINT tran3;
UPDATE emp1 SET job='이사' WHERE empno = 1006;
SELECT * FROM emp1 WHERE empno = 1006;
...
SELECT @deptno := deptno FROM dept WHERE dname = '경리부';
INSERT INTO emp (empno, deptno, job, ename, hiredate, sal) VALUES( @maxempno +1 , @deptno , '사원', '송중기', CURDATE(), 500);
...
CREATE TABLE table1 AS SELECT * FROM emp;
...
CREATE TABLE table2 LIKE emp;  -- 구조만 복사
...
SELECT @deptno1 := deptno FROM dept WHERE dname = 'HR팀';
SELECT @deptno2 := deptno FROM dept WHERE dname = '인사부';
   
UPDATE emp SET deptno = @deptno2 , sal = 300 WHERE deptno = @deptno1 ; 


UPDATE emp SET deptno = (SELECT deptno FROM dept WHERE dname = '인사부') , sal = 300 

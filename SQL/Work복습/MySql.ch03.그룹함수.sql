...
SELECT MAX(sal), MIN(sal) FROM emp WHERE deptno = 10 UNION
...
SELECT deptno, COUNT(*), COUNT(IF(comm=0, null, comm)) FROM emp GROUP BY deptno ORDER BY deptno ASC;
...
SELECT job, SUM(sal) FROM emp GROUP BY job, sal WITH ROLLUP;
...
-- -----------------------------------
SELECT deptno, AVG(sal) FROM emp WHERE deptno in(10, 20, 30) GROUP BY deptno HAVING AVG(sal) >= 400 ORDER BY deptno ASC;
...
SELECT job, SUM(sal) FROM emp WHERE job != '사원' GROUP BY job HAVING SUM(sal) >= 1000 ORDER BY SUM(sal) ASC;
...
-- 미션 4. 과장의 수를 출력하시오.
SELECT COUNT(job) FROM emp WHERE job = '과장';
